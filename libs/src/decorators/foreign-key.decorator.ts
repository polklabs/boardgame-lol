...
export function ForeignKey<T>(entityClass: { new (partial: Partial<T>): T }) {
  return function (target: any, propertyKey: string) {
    const keys: ForeignKeyType = Reflect.getOwnMetadata(foreignKey, target.constructor) ?? {};
...
export function getForeignKeys(target: any) {
  const toReturn = Reflect.getOwnMetadata(foreignKey, target) ?? {};
  return Object.keys(toReturn);
}

export function getForeignKey(target: any, propertyKey: string) {
  const toReturn = Reflect.getOwnMetadata(foreignKey, target.constructor) ?? {};
